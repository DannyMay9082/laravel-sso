...
namespace AdlyAlimin\LaravelSSO;

use Illuminate\Database\Eloquent\ModelNotFoundException;
use Illuminate\Support\Facades\Auth;
use Illuminate\Support\Facades\Cache;
use Illuminate\Support\Facades\Session;
use AdlyAlimin\LaravelSSO\Traits\SSOServerTrait;
use Zefy\SimpleSSO\SSOServer;
use AdlyAlimin\LaravelSSO\Resources\UserResource;
    ...
    protected function getUserInfo(string $email)
    {
        try {
            if(config('laravel-sso.usingRelation') == true) {
                $user = config('laravel-sso.usersModel')::where('email', $email)->with("config('laravel-sso.relationName')")->firstOrFail();
            } else {
                $user = config('laravel-sso.usersModel')::where('email', $email)->firstOrFail();
            }
        } catch (ModelNotFoundException $e) {
